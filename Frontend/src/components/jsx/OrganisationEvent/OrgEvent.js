...
function OrgEvent() {
  ...
  const handlepostdetails = (post) => {
    ...
    navigate("/organizationevents/detailedview", {
  ...
  const handlesearchSubmit = async (event) => {
    ...
    try {
      ...
      const response = await api.post("/organizationeventpostsbytitle/", data);
      setDetails(response.data);
      // console.log(response.data);
    } catch (error) {
      console.error("Error fetching details:", error);
  ...
  const handleusereventfeedback = (post) => {
    navigate("/organizationevents/orgeventfeedback", {
      state: JSON.stringify(post),
    });
  }
  ...
  return (
    <>
      ...
      {/* <div>
        <Link to="/organizationevents/addpost">
